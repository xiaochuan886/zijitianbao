...
const withdrawalRequestSchema = z.object({
  ...
  recordType: z.enum(["actual_user", "actual_fin"]),
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("获取实际支出记录失败", error);
    return NextResponse.json({ error: "获取实际支出记录失败" }, { status: 500 });
  }
}
...
async function handleAllRecords(params: any) {
  ...
  // 根据记录类型选择表
  const recordModel = params.recordType === "user" ? prisma.actualUserRecord : prisma.actualFinRecord;

  // 查询数据库中的记录
  const dbRecords = await recordModel.findMany({
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.action === "withdrawal") {
      ...
      const { recordId, reason, recordType } = result.data;
      
      // 根据记录类型选择服务
      const recordService = recordType === "actual_user" 
        ? services.actualUserRecord 
        : services.actualFinRecord;
    ...
    console.error("处理实际支出记录请求失败:", error);
    return NextResponse.json({ 
      error: "处理实际支出记录请求失败", 
