...
const requestSchema = z.object({
  ...
  recordType: z.enum(["user", "finance"]).default("user")
});
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { records, recordType } = validationResult.data;
    const recordModel = recordType === "user" ? prisma.actualUserRecord : prisma.actualFinRecord;
    ...
    console.log(`处理${records.length}条${recordType}记录，其中临时ID: ${tempRecords.length}，真实ID: ${realRecords.length}`);
    ...
    if (realRecords.length > 0) {
      for (const record of realRecords) {
        try {
          ...
          // 更新记录
          await (recordModel as any).update({
            where: { id: record.id },
            data: {
              amount,
          ...
          if (parts.length >= 4 && parts[0] === 'temp') {
            ...
            if (detailedFundNeedId && !isNaN(year) && !isNaN(month)) {
              ...
              if (detailedFundNeed) {
                ...
                // 创建新记录
                await (recordModel as any).create({
                  data: {
                    year,
                    month,
                    amount,
