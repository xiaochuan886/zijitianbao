...
### 2025-02-26
- **修复**: 解决了撤回申请功能无法正常工作的问题
  - 修复了客户端API调用404错误的问题，通过更新API路径为 `/api/funding/predict/withdrawal-v2`
  - 改进了撤回API，使其能够接受项目ID并自动查找关联的记录
  - 修复了对话框不自动关闭的问题，确保无论API成功或失败都会关闭对话框
  - 增强了API错误处理，提供更详细的错误信息
  - 修复了前端状态显示问题，确保撤回申请待审批状态正确显示
- **功能调整**: 改进了项目状态管理和编辑权限控制
  - 修改了编辑权限规则：只有状态为"草稿"和"未填写"的项目才能编辑
  - 优化了批量填报功能：当选中的项目中包含不可编辑的项目时，批量填报按钮会被禁用
  - 在状态筛选选项中添加了"撤回审核中"状态选项，方便用户筛选
  - 调整了操作菜单，根据项目状态动态显示可用操作
